...
function QuoteStatus({ status }) {
  const normalized = String(status || "Borrador").toLowerCase();
  const tone = normalized === "convertida"
  ...
  return (
    <span className={`rounded-full px-2.5 py-1 text-[0.65rem] font-black uppercase ${tone}`}>
      {status || "Borrador"}
    </span>
  );
}
...
function QuotePanel() {
  ...
  const loadQuotes = useCallback(async () => {
    try {
      setLoading(true);
      const response = await getRecords("quote", {
        page: 1,
        limit: 50,
        sortBy: "quote_date",
        sortOrder: "desc",
      ...
      console.error(error);
      addToast("No se pudieron cargar las cotizaciones", "error");
    } finally {
      setLoading(false);
    }
  }, [addToast]);
  ...
  const filteredQuotes = useMemo(() => {
    ...
    return quotes.filter((quote) =>
      [quote.name, quote.status, quote.type, quote.quote_date]
    ...
    return (
      <section className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm">
        <button
          type="button"
          onClick={openList}
          className="mb-4 inline-flex items-center gap-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-800"
        >
          <ArrowLeft size={17} /> Cotizaciones
        </button>
        <div className="mb-3 px-1">
          ...
          <h2 className="mt-1 text-xl font-black text-slate-950">
            {selectedQuoteId ? "Ver y editar cotizacion" : "Crear cotizacion"}
          </h2>
        </div>
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-start justify-between gap-3 px-1">
          <div>
            ...
            <h2 className="mt-1 text-xl font-black text-slate-950">Cotizaciones recientes</h2>
          </div>
          <button
            type="button"
            onClick={() => {
            ...
            className="rounded-2xl bg-cyan-400 px-4 py-2.5 text-sm font-black text-slate-950"
          >
            Nueva
          </button>
        </div>
        ...
        <label className="relative mt-4 block">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-slate-200 py-3 pl-10 pr-3 text-sm"
          />
        </label>
        ...
        <div className="mt-4 space-y-3">
          {filteredQuotes.map((quote) => (
            <button
              ...
              className="w-full rounded-2xl border border-slate-200 p-4 text-left transition active:bg-slate-50"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate font-black text-slate-950">
                    {quote.name || "Cotizacion sin nombre"}
                  </p>
                  <p className="mt-1 text-xs font-semibold text-slate-500">
                    {formatShortDate(quote.quote_date || quote.createdAt)} · {quote.type || "Contado"}
                  </p>
                </div>
              ...
              <div className="mt-3 flex items-end justify-between border-t border-slate-100 pt-3">
                <div>
                  ...
                  <p className="mt-1 font-black text-slate-900">{formatCRC(quote.total)}</p>
                </div>
                <span className="text-sm font-black text-cyan-700">Abrir</span>
              </div>
            </button>
          ))}
          ...
          {!loading && filteredQuotes.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center">
              <FilePlus2 className="mx-auto text-slate-400" />
              <p className="mt-2 font-bold text-slate-700">
                {search ? "No hay coincidencias" : "Todavia no hay cotizaciones"}
              </p>
            </div>
          ) : null}
        </div>
