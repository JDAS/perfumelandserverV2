...
function sanitizeField(rawField = {}, index = 0) {
  ...
  return {
    label,
    apiName,
    type: FIELD_TYPES.includes(rawField.type) ? rawField.type : "text",
    required: Boolean(rawField.required),
    options:
      rawField.type === "select"
        ? Array.from(new Set((rawField.options || []).map((opt) => String(opt).trim()).filter(Boolean)))
        : [],
...
function ensureDefaultListViews(listViews = [], fields = []) {
  if (Array.isArray(listViews) && listViews.length > 0) {
    const sanitized = listViews.map((view, index) => ({
      ...
      apiName: normalizeApiName(view.apiName || view.label || `view_${index + 1}`),
      isDefault: Boolean(view.isDefault),
      columns: Array.isArray(view.columns) && view.columns.length > 0 ? view.columns : fields.slice(0, 5).map((field) => field.apiName),
...
function sanitizeObjectPayload(payload = {}, existingObject = null) {
  const name = String(payload.name || existingObject?.name || "Nuevo Objeto").trim();
  const apiName = normalizeApiName(payload.apiName || existingObject?.apiName || name);

  const fieldsInput = Array.isArray(payload.fields) && payload.fields.length > 0
    ? payload.fields
  ...
  const layoutInput = Array.isArray(payload.layout) && payload.layout.length > 0
    ? payload.layout
  ...
  const layout = layoutInput.map((layoutItem, index) => ({
    ...
    apiName: normalizeApiName(layoutItem.apiName || layoutItem.label || `layout_${index + 1}`),
    sections: Array.isArray(layoutItem.sections) && layoutItem.sections.length > 0
  ...
  const listViews = ensureDefaultListViews(payload.listViews ?? existingObject?.listViews, fields);

  return {
    name,
    pluralLabel: String(payload.pluralLabel || existingObject?.pluralLabel || name).trim(),
    description: String(payload.description || existingObject?.description || "").trim(),
    apiName,
...
function validateObjectMetadata(payload = {}) {
  ...
  for (const field of payload.fields || []) {
    ...
    if (!field.apiName) {
      errors.push(`El campo ${field.label || "sin nombre"} debe tener apiName`);
      continue;
    }
    ...
    if (!FIELD_TYPES.includes(field.type)) {
      errors.push(`El tipo ${field.type} no es válido para el campo ${field.apiName}`);
    }
  ...
  for (const view of payload.listViews || []) {
    ...
    if (view.sortBy && view.sortBy !== "createdAt" && view.sortBy !== "updatedAt" && !fieldApiNames.has(view.sortBy)) {
      errors.push(`El campo de orden ${view.sortBy} de la vista ${view.apiName} no existe`);
    }
  }
